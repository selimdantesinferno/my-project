...
> **🚫 ABANDONED — 2026-05 검증 결과: 최신 CapCut은 `template` 파일을 암호화해 저장한다.**
> base64 위에 암호화된 바이트(엔트로피 ≈ 균일)가 들어있어, 우리가 만든 JSON을
> 직접 쓰는 방식은 불가능. 도구는 대신 **컷 분할 mp4 + SRT 번들** 출력으로
> 전환했다 (`export/bundle.py`). 이 문서는 향후 JianYing(剪映, 평문 JSON)
> 경로를 검토하거나 모드 2의 효과 자막 스키마를 만들 때를 위해 보존한다.

이 문서는 CapCut 데스크톱 프로젝트 파일 구조를 리버스 엔지니어링하며
정리한 작업 노트입니다. **공식 문서가 아니며 버전마다 다릅니다.**
